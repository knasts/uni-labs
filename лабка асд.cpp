...
/*Варіант 14
Структура Додаткова сесія має наступні поля: кількість незакритих предметів,
дедлайн, шанс відрахування. Створити список неприємних речей в якому
можна швидко знайти додаткову сесію.*/
static long long generateRandLong()
{
    int amount = (rand() % 4) + 10; 
    long long number = 0;
    long long j = 1;
...
bool testHashTable()
{
    ...
    cout << "Time: " << myTime << " , after insert: " << myInsertSize << " , after erase: "
         << myEraseSize << ", found: " << myFoundAmount << endl;
        cout << "STL unordered_map:" << endl;
    cout << "Time: " << stlTime << " , after insert: " << stlInsertSize << " , after erase: "
         << stlEraseSize << ", found: " << stlFoundAmount << endl << endl;
        delete[] keys;
    delete[] keysToInsert;
    delete[] keysToErase;
    delete[] keysToFind;
